...
def main(argv=None):
    ...
    failed, without, flagged, warnings = [], [], [], 0
    for directory in directories:
        ...
        if not report.ok:
            failed.append(directory.name)
        # The physics pre-check is reported, never enforced: an inconsistent
        # verdict is a question for the user, not a build failure (ticket 09).
        verdict = physics.check(spec).verdict
        if verdict == "INCONSISTENT":
            flagged.append(directory.name)
    ...
    if flagged:
        print(f"  physics pre-check flagged (not a failure): {', '.join(flagged)}")
    verdict = "FAIL" if failed else "PASS"
    print(f"{verdict}: validate_cases specs={total} failed={len(failed)} "
          f"warnings={warnings} precheck_flagged={len(flagged)} "
          f"without_spec={len(without)}")
