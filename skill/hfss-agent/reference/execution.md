...
13. **Result QA** — check the agreed signals; report each with numbers or explicitly "unreadable — flaky readout" where the readout fails, and **escalate to a fresh desktop process before concluding anything about the pairing** (EC#6): read through `read_results.ReadoutSession(hfss, recycle=ws_common.recycle_desktop)`, which spends one `recycle_desktop()` per run and no more. A re-attach is not an escalation — `attach()` reconnects by the pinned port and the bounded connect re-pins only a *dead* desktop, so a degraded channel survives it untouched. Each signal's verdict line in `results/state/readouts.txt` carries its `route=` token (`live-channel` / `fresh-process` = degradation confirmed / `both-failed` = systematic on this pairing / `untested` = no fresh process ran, which proves nothing and must never be written up as a pairing verdict).
