...
1. **Clarification** — deliver one block: request minimums, missing critical setup features, proposed Recipe + Assumptions, proposed Result QA signals; lock the parameters/variables. Record the confirmation verbatim in the summary, then **write the State ledger**. First run the `analyze-papers` skill on `knowledge/reference-papers/Global/` (always — the general geometry, port and setup guidance) and on the one project folder that matches the requested structure (`Bowtie Patch/`, `U-slot/`, … — the design's own dimensions, feed and measured results), one invocation per folder, and read the resulting agent notes before drafting the block (SKILL.md "Read first" #6). The other project folders stay unread; no match, or an ambiguous one, is stated in the block rather than resolved silently. The notes are context for the Recipe and QA signals, never automatic playbook material.
