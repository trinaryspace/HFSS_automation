...
14. **Summary** — record the outcome first, by script, never by hand: `python scripts/record_outcome.py --workspace workspaces/<name> --outcome completed|escalated|abandoned --completions <n> --note "<user verdict verbatim>"` writes `results/state/outcome.txt` in the key=value form the run card parses and refuses any other outcome word (the last run wrote free text there and its card read `unrecorded`). Then write `summary.md`: acute decisions, the Model description, Recipe + signals used, any deltas recorded by sync; have `runcard` draft the `## Run card` from `state.md`, `results/state/*.txt` and `results/`, revise, and append it via the measurement harness.
