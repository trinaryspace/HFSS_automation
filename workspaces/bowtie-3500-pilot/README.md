...
- **The solve runs under a detached watchdog (ADR 0006).** `08_solve.py`
  cleans stale results (only real solve results; asol skeletons are not),
  probes for an in-flight solve, submits `analyze(blocking=False)`,
  launches `python src/poll_solve.py <name>.aedt` fully detached
  (`subprocess.Popen(DETACHED_PROCESS)` — the pilot's deterministic
  launch shape), and exits. The watchdog appends one line to
  `results/state/solve_progress.txt` every ~20 s (status `running` /
  `settling` / `complete` / `stalled`; optionally pass an expected
  per-frequency-count via the `EXPECTED_SD` env var, but settle-mode is
  the default — do NOT guess a count, this box's sweep writes `F###`
  dirs, not `.sd` files) and exits on completion or stall. The agent
  only reads the state file — never foreground-polls, never estimates.
