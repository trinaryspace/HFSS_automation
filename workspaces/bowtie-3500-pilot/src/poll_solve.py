...
def scan_results(root):
    ...
    in `.asol` or `.sd` — both directories AND files (HFSS writes the
    per-sweep-point outputs as `<variant>.sd` FILES and some derived
    results as `.asol`/`.sd` dirs; prior art counted files; counting both
    keeps EXPECTED_SD truthful either way) — the total number of regular
    files, and their summed size. A single os.walk — no recursive
