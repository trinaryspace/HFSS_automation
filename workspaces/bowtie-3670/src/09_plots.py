...
Recipe plots: |S11| (dB) vs freq, VSWR vs freq, far-field gain pattern
(2D polar cuts at 3.5 GHz; 3D attempt). Readout is flaky (EC#6): retry
with backoff, re-attach if needed, and write what was observed.
"""
...
def main() -> int:
    ...
    if s11 is None:
        ...
        plt.title("Bowtie patch |S11|, paper replica (no DGS)")
        plt.grid(True)
    ...
    if vswr is None:
        ...
        plt.title("VSWR — bowtie patch, paper replica (no DGS)")
        plt.grid(True)
