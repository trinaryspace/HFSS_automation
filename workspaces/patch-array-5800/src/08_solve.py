...
Workspace-local solve launcher (the template does not ship it; the shape is
patch-2400's delivered `08_solve.py`, kept verbatim in behavior):

1. Cleanup: stale solve results removed (skip-if-no-stale: with no real
   results the raw DeleteFullVariation surface errors — benign).
2. In-flight-solve probe: results-dir age + live solver processes; when a
   solve looks live the session must ASK the user before submitting (the
   WARNING line below is the probe's output — it never submits silently).
3. Submit: analyze(setup="Setup1", blocking=False) — True is submission
   only, not completion (EC#5); never foreground-poll; never estimate.
...
The design solved is the one ws_common.DESIGN names (ElementsOnly for the
stage-1 extraction; PatchArray for the fed run) — switch the constant and
re-run for the second solve.
"""
...
def main() -> int:
    ...
    if stale:
        ...
        try:
            ...
            print("cleanup_solution raised (benign on first solve): %s: %s" % (type(exc).__name__, str(exc)[:200]), flush=True)
    else:
